...
def computeDelta(wt, X, Xi):
    ...
    def calculate_similarity(x, y):
        ...
        for i in range(0, len(x)):
            similarity += (x.iloc[i] - mean_x) * (y.iloc[i] - mean_y)

        return similarity/(len(x) * np.std(x) * np.std(y));
    ...
    for i in range(0,len(Xi)):
        y = Xi.iloc[i][:-1]
        similarity_exp = np.exp(wt * calculate_similarity(x, y))
        numerator += Xi.iloc[i][-1]*similarity_exp
        denominator += similarity_exp

    avg_price_change = numerator/denominator
